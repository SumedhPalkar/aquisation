...
const securityMiddleware = async (req, res, next) => {
  try {
    ...
    if(decision.isDenied() && decision.reason.isBot()){
      ...
      return req.status(403).json({error: 'Forbidden', message:'Automated requests are not allowed'}); 
    }

    if(decision.isDenied() && decision.reason.isShield()){
      logger.warn('Shield blocked request', {ip: req.ip, userAgent: req.get('User-Agent'), path: req.path, method: req.method});

      return req.status(403).json({error: 'Forbidden', message:'Requests blocked by security policy'}); 
    }

    if(decision.isDenied() && decision.reason.isRateLimit()){
      logger.warn('Rate limit exceeded', {ip: req.ip, userAgent: req.get('User-Agent'), path: req.path });

      return req.status(403).json({error: 'Forbidden', message}); 
    }
